...
def fetch_axfr(conf: Dict[str, Any], full_conf: Dict[str, Any], **kwargs) -> None:
    for zone_data in conf:
        ...
        tsig_args = ['-k', f'tsig/{zone}tsig'] if zone_data['tsig'] else []
        zone_args = ['dig', *tsig_args, f'@{ns}', '+noall', '+answer', '+noidnout', '+onesoa', '-t', 'AXFR', zone]
...
def get_fetch(conf: Dict[str, Any]) -> Callable[[str], Any]:
    ...
    auth_res = requests.post(f'{account_baseurl}/api/authenticate', json=conf['credentials'])
